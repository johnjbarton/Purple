...
function joinStacks(current) {
  var stacked = {};
  Error.captureStackTrace(stacked, joinStacks);
  // delete q stuff
  var stack = ['dummy']; /*stacked.stack.split('\n').filter(function(line) {
    return (line.indexOf('q/q.js') === -1) && 
           (line.indexOf('q-comm/q-comm.js') === -1) && 
           (line.indexOf('(native)') === -1);
  });
  */
  stack.shift(); // remove the [Object object] or error message
  return current ? current.stack.concat(stack) : stack;
}
...
*/    // browsers
    if (typeof MessageChannel !== "undefined") {
        ...
        nextTick = function (task) {
            ...
            var stack = joinStacks(current);
            tail = tail.next = {task: task, prev: current, file: file, stack: stack};
