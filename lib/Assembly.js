...
Assembly.addPartContainer = function(extendMe, bootstrap) {
  ...
  extendMe._partContainer = {
    ...
    append: function(part) {
      ...
      if (this.toEachPart) {
        // orginally I called this 'change' and signalled the kind of change in the mutation field.
        // But that makes the dependent code branch on mutation and it makes the method name too generic.
        this.toEachPart('partAdded', [{containerName: this.name, mutation: 'add', propertyName: part.name, value: part}]);
      }
    },
    remove: function(part) {
      ...
      if (this.toEachPart) {
        this.toEachPart('partDeleted', [{containerName: this.name, mutation: 'delete', propertyName: part.name, value: part}]);
      }
    }
  };
