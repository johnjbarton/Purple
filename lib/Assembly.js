...
Assembly.addPartContainer = function(extendMe, bootstrap) {
  ...
  extendMe._partContainer = {
    ...
    append: function(part) {
      ...
      if (this.toEachPart) {
        this.toEachPart('change', [{containerName: this.name, mutation: 'add', propertyName: part.name, value: part}]);
      }
    },
    remove: function(part) {
      ...
      if (this.toEachPart) {
        this.toEachPart('change', [{containerName: this.name, mutation: 'delete', propertyName: part.name, value: part}]);
      }
    }
  };
