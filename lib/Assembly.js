...
Assembly.addPartContainer = function(extendMe) {
  ...
  extendMe.registerPart = function(part) {
    if (part) {
      ...
      } else if (this._partContainer.partsByName.hasOwnProperty(part.name)) {
        console.warn("Assembly: register part "+part.name+" twice");
      } else {
        this._partContainer.partsByName[part.name] = part;
  ...
  extendMe.unregisterPart = function(part) {
    ...
    delete this._partContainer.partsByName[part.name];
  };
