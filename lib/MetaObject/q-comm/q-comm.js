    ...
    if (port.postMessage) {
        ...
        function oneShotIntro(event) {
            if (event.source !== window) {
                ...
                debug(window.location.toString().split('/').pop()+" Received introduction from "+event.source.location+' '+event.data.QCOMM, event);
                ...
                if (event.data.QCOMM === 'HELLO') {
                    debug(window.location.toString().split('/').pop()+" Sending acknowledgement to "+port.location, event);
        ...
        debug(window.location.toString().split('/').pop()+" Sending introduction to "+port.location);
