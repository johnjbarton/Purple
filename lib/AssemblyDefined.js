...
Assembly.addPartContainer = function(extendMe, bootstrap) {
  ...
  extendMe._partContainer = {
    ...
    append: function(part) {
      ...
      if (Object.keys(this.partsByName).length !== this.parts.length) {
        console.error("Assembly ERROR parts and partsByName out of sync", this);
      }
    },
    remove: function(part) {
      var index = this.parts.indexOf(part);
      if (index === -1) {
        console.warn("Assembly: unregisterPart called for part not regisitered", {extendMe: extendMe, part: part});
        return;
      }
