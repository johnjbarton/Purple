...
Assembly.addPartContainer = function(extendMe, bootstrap) {
  ...
  extendMe._partContainer = {
    ...
    append: function(part) {
      ...
      extendMe.toEachPart('partAdded', [{assembly: extendMe, containerName: this.name, mutation: 'add', propertyName: part.name, value: part}]);
      if (part.partAdded) {
        // Notify the new part of all the existing parts
        extendMe.forEachPart(function toNewPart(existingPart) {
          part.partAdded({assembly: extendMe, containerName: this.name, mutation: 'add', propertyName: existingPart.name, value: existingPart});
        });
      }
      if (this.toEachPart) {  
        // Notify the parts container listeners 
        this.toEachPart('partAdded', [{assembly: extendMe, containerName: this.name, mutation: 'add', propertyName: part.name, value: part}]);
      }
    },
    remove: function(part) {
      ...
      this.toEachPart('partDeleted', [{assembly: extendMe, containerName: this.name, mutation: 'delete', propertyName: part.name, value: part}]);
      if (this.toEachPart) {
        this.toEachPart('partDeleted', [{assembly: extendMe, containerName: this.name, mutation: 'delete', propertyName: part.name, value: part}]);
      }
